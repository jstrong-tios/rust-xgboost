...
impl Booster {
    ...
    pub fn serialize_to_buffer(&mut self) -> XGBResult<Vec<u8>> {
        ...
        xgb_call!(
            xgboost_sys::XGBoosterSerializeToBuffer(
                self.handle, 
                &mut out_len,
                &mut out_result,
            )
        ...
        let data: Vec<i8> = unsafe { slice::from_raw_parts(out_result, out_len as usize).to_vec() };
    ...
    pub fn unserialize_from_buffer(bytes: &[u8]) -> XGBResult<Self> {
        ...
        xgb_call!(xgboost_sys::XGBoosterUnserializeFromBuffer(handle, bytes.as_ptr() as *const _, bytes.len() as u64))?;
        Ok(Booster { handle })
    }
...
mod tests {
    ...
    fn serialize_and_unserialize_from_buffer() {
        ...
        let bytes = booster.serialize_to_buffer().unwrap();
        assert!(bytes.len() > 0);

        drop(booster);
        let booster = Booster::unserialize_from_buffer(&bytes[..]).expect("load booster from buffer");
        let attr = booster.get_attribute("foo").expect("Getting attribute failed");
        assert_eq!(attr, Some("bar".to_owned()));
    }
